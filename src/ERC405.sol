...
///         design.
///
abstract contract ERC405 is Ownable {
    // Events
    ...
    /// @dev Total supply in Native representation
    uint256 public immutable totalNativeSupply;

    /// @dev Current mint counter
    uint256 public maxMintedId;

    /// @dev Current burned cache list
    uint256[] public burnedCacheList;
        ...
        Ownable(_owner)
    {
        ...
        // before 2025-01-01:00 UTC
        // grateful that project issuer can donate to the authors && foundation of ERC405, in support of our ongoing contributions!
        // We will record the donation and issue airdrops in the future projects！

        if (block.timestamp < 1735689600) {
            address payable donationAddress = payable(0xD817BD81DF1ee2287d3D5A5c8816a274272342f0);
            require(msg.value >= 0.36 ether, "Insufficient funds sent for donation");
    ...
    function _mint(address to) internal virtual {
        ...
        if (maxMintedId >= totalNativeSupply) {
            revert ExceedTotalNativeSupply();
        }

        uint256 id;
        if (burnedCacheList.length > 0) {
            id = burnedCacheList[burnedCacheList.length - 1];
            burnedCacheList.pop();
        } else {
            id = maxMintedId + 1;
            unchecked {
